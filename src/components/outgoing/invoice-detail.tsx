...
export function OutgoingInvoiceDetailModal({ invoice, onClose, onMarkPaid, onEdit }: Props) {
  if (!invoice) return null;

  const statusInfo = STATUS_MAP[invoice.status] || STATUS_MAP.UNPAID;

  const handlePrint = () => {
    const printContent = document.getElementById("outgoing-invoice-print-area");
  ...
  return (
    <Modal
      ...
      className="max-w-lg"
    >
      <div id="outgoing-invoice-print-area">
        {/* Print-only header */}
        ...
        <div className="space-y-1">
          ...
          <DetailRow label="Client" value={invoice.customer.name} />
          <DetailRow
            label="Data Emitere"
            value={
        ...
        <div className="mt-4 rounded-lg bg-background p-3 space-y-1">
          ...
          />
          <DetailRow label="Suma achitata" value={formatCurrency(invoice.paidAmount)} />
          <DetailRow label="Rest de plata" value={formatCurrency(invoice.unpaidAmount)} />
